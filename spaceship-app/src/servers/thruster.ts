    ...
    async ({ power_level }) => ({
      content: [{
        ...
        text: JSON.stringify({
          ...
          message: `Thrust level set to ${power_level}%. Temperature normalising — estimated ${Math.round(200 + power_level * 6.5)}°C.`,
        }, null, 2),
      }],
    }),
  );
    ...
    async () => ({
      content: [{
        ...
        text: JSON.stringify({
          ...
          message: "Emergency shutdown complete. All thruster systems offline.",
        }, null, 2),
      }],
    }),
  );
